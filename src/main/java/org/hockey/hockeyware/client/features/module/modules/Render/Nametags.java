...
import org.hockey.hockeyware.client.util.render.ColorUtil;
import org.lwjgl.opengl.GL11;

import java.awt.*;
import java.util.Objects;

import static org.hockey.hockeyware.client.util.render.ColorUtil.getHealthColor;
...
public class Nametags extends Module {
    ...
    public static final Setting<Boolean> entchantments = new Setting<>("Entchantments", false);
    ...
    public Nametags() {
        super("Nametags", "Allows You To Modify Player Nametags", Category.Render);
        INSTANCE = this;
    }
    ...
    private void renderNametag(final EntityPlayer player, final double x, final double y, final double z) {
        GL11.glPushMatrix();
        String name = (getHockey().friendManager.isFriend(player.getUniqueID()) ? ChatFormatting.AQUA : ChatFormatting.WHITE) + player.getName();
        ...
        if (CustomFont.INSTANCE.isOn()) {
            HockeyWare.INSTANCE.fontManager.drawStringWithShadow(name, (float) (-width), 10.0f, getHealthColor(player));
        } else {
